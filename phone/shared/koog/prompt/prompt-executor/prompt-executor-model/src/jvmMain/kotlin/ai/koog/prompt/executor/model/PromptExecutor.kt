...
public actual abstract class PromptExecutor actual constructor() : PromptExecutorAPI {
    ...
    public fun executeBlocking(
        ...
        execute(prompt, resolvedModel, tools)
    }
    ...
    public fun executeMultipleChoicesBlocking(
        ...
        executeMultipleChoices(prompt, resolvedModel, tools)
    }
    ...
    /**
     * Executes a given prompt against an already-resolved model and returns its response as a synchronous stream of [StreamFrame] objects.
     *
     * @param prompt The [Prompt] object containing the input messages and parameters for the execution.
     * @param resolvedModel The [ResolvedModel] returned by [resolveModel].
     * @param tools An optional list of [ToolDescriptor] objects defining the tools available during execution. Defaults to an empty list.
    ...
    public fun executeStreamingBlocking(
        ...
        resolvedModel: ResolvedModel,
        tools: List<ToolDescriptor> = emptyList(),
    ): Publisher<StreamFrame> = executeStreaming(prompt, resolvedModel, tools).asPublisher()
    ...
    public fun moderateBlocking(
        ...
        moderate(prompt, resolvedModel)
    }

    /**
     * Resolves the requested [requestedModel] into the [ResolvedModel] to use for the given [promptExecutorOperation].
     *
    ...
    @OptIn(InternalKoogUtils::class)
    @JavaAPI
    @JvmName("resolveModel")
    public fun resolveModelBlocking(
        requestedModel: LLModel,
        promptExecutorOperation: PromptExecutorOperation
    ): ResolvedModel = runBlockingReentrant {
        resolveModel(requestedModel, promptExecutorOperation)
    }
